...
```
 ...
 semgrep ─────> SWEEP ──> 100 raw findings ──────┘        │
                                                          ▼
                                    LLM + human review ── TRIAGE with Superlinked Qwen-3.8-27B
                                                          │
                       ┌──────────────────────────────────┼───────────────────┐
                       ▼                                  ▼                   ▼
                  25 "tp" (model)                  72 "fp" (model)      3 uncertain
                       │
                       ▼
              DIFFERENTIAL GATE (verify: attack replay vs benign control, ×3) <---- AUGMENT with independent bugfinder swarms
                       │
        ┌──────────────┼──────────────┐
        ▼              ▼              ▼
...
| Model-triaged (Superlinked Qwen-3.8-27b) | 100    | 25 tp · 72 fp · 3 uncertain                |
...
early aft    variant swarm reviews ~100 seeds by hand  → 25 research candidates
             triage run on the 100 semgrep candidates, using Superlinked's
             Qwen-3.8-27b (run record: .funnel/triage-runs/triage-qwen38-*)
...
arness-blocked vibe-kanban authz claims (#133–#135)
